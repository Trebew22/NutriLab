...
def all_is_valid(request, password, confirm_password, email, username):
    ...
    if not re.search('[\w]+@[\w]+\.com[\.\w]{0,5}', email):
        messages.add_message(request, constants.ERROR, 'Email precisa ser válido')
        return False

    if User.objects.filter(username = username).exists():
        messages.add_message(request, constants.ERROR, 'Seu Usuário ja existe')
        return False
