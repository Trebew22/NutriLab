...
from .utils import email_html, all_is_valid
...
def cadastro(request):
    if request.method == "GET":
        ...
        if not all_is_valid(request, senha, confirmar_senha, email, username):
...
def ativar_conta(request, token):
    ...
    if token.ativo:
        messages.add_message(request, constants.WARNING, 'Esse token ja foi usado')
        return redirect('/auth/logar')
